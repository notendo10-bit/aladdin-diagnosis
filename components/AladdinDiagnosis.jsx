...
const TYPES = {
  rousseau:  { img:"/types/rousseau.webp",  name:"ルソー型",         nameEn:"Rousseau",  sub:"自然と自由の革命家",          color:"#2d7a3a", light:"#e8f5e9", desc:"社会の矛盾に怒りを感じ、人間本来の自由と平等を取り戻そうとする。感情と直感を信じ、既存の秩序に疑問を投げかける革命的な魂。", keyword:["自由","平等","自然","革命"], emoji:"🌿" },
  gandhi:    { img:"/types/gandhi.webp",    name:"ガンジー型",        nameEn:"Gandhi",    sub:"非暴力の精神的指導者",        color:"#e65100", light:"#fff3e0", desc:"暴力ではなく魂の力で世界を変えようとする。忍耐と信念を持ち、どんな困難にも精神的な強さで立ち向かう。平和こそが最強の武器だと知っている。", keyword:["非暴力","精神","忍耐","真理"], emoji:"☮️" },
  marx:      { img:"/types/marx.webp",      name:"マルクス型",        nameEn:"Marx",      sub:"構造から変える革命思想家",    color:"#c62828", light:"#ffebee", desc:"問題の根っこは個人ではなく社会の構造にあると見抜く。格差・不平等・権力の仕組みを鋭く分析し、根本的な変革を求める。", keyword:["構造","格差","変革","平等"], emoji:"✊" },
  nietzsche: { img:"/types/nietzsche.webp", name:"ニーチェ型",        nameEn:"Nietzsche", sub:"価値を打ち壊す超人",          color:"#4a148c", light:"#f3e5f5", desc:"既存の道徳や常識を疑い、自分だけの価値観を創造しようとする。強さと創造性を最も重視し、群衆の価値観に流されない孤高の精神を持つ。", keyword:["超人","創造","力への意志","価値転換"], emoji:"⚡" },
  confucius: { img:"/types/confucius.webp", name:"孔子型",            nameEn:"Confucius", sub:"礼と調和の実践者",            color:"#bf360c", light:"#fbe9e7", desc:"人と人の関係を大切にし、礼儀と秩序によって社会を良くしようとする。伝統の中に智慧を見出し、教育と人格の陶冶を最も重要と考える。", keyword:["礼","調和","教育","仁"], emoji:"🏮" },
  socrates:  { img:"/types/socrates.webp",  name:"ソクラテス型",      nameEn:"Socrates",  sub:"問い続ける対話の哲人",        color:"#1565c0", light:"#e3f2fd", desc:"答えより問いを大切にする。「知らないことを知っている」という姿勢で対話を通じて真理に迫る。人々に考えることを促す産婆術の使い手。", keyword:["問い","対話","無知の知","真理"], emoji:"❓" },
  plato:     { img:null,                    name:"プラトン型",        nameEn:"Plato",     sub:"理想世界を夢見る哲人王",      color:"#00695c", light:"#e0f2f1", desc:"現実の背後に完全な理想の世界があると信じる。現状に妥協せず、あるべき社会の姿を思い描き、知恵ある者が導くべきだという強い信念を持つ。", keyword:["理想","哲人王","イデア","正義"], emoji:"👑" },
  aristotle: { img:null,                    name:"アリストテレス型",  nameEn:"Aristotle", sub:"中庸と実践の万能学者",        color:"#4e342e", light:"#efebe9", desc:"極端を避け、バランスと中庸を重んじる。理論だけでなく実践を大切にし、あらゆる分野に好奇心を持って探求する。幸福とは徳の実践だと考える。", keyword:["中庸","実践","徳","幸福"], emoji:"⚖️" },
  kant:      { img:null,                    name:"カント型",          nameEn:"Kant",      sub:"普遍的道徳の番人",            color:"#283593", light:"#e8eaf6", desc:"「すべての人に当てはまるルールか？」を常に問う。感情や損得ではなく、義務と理性によって行動する。普遍的な道徳法則を何より大切にする。", keyword:["義務","普遍","理性","道徳"], emoji:"📐" },
  mill:      { img:null,                    name:"ミル型",            nameEn:"Mill",      sub:"最大幸福を追求する自由主義者",color:"#2e7d32", light:"#e8f5e9", desc:"最も多くの人が最も幸せになれる選択を重視する。個人の自由を守りながら社会全体の幸福を最大化しようとする、バランス感覚に優れた実践的思想家。", keyword:["功利","自由","幸福","社会改革"], emoji:"🕊️" },
  darwin:    { img:null,                    name:"ダーウィン型",      nameEn:"Darwin",    sub:"変化と適応の観察者",          color:"#558b2f", light:"#f1f8e9", desc:"世の中は常に変化し、適応したものが生き残ると知っている。固定観念を持たず、観察と証拠を積み重ねて真実に迫る。変化を恐れず、むしろ楽しむ。", keyword:["進化","変化","適応","観察"], emoji:"🔬" },
  buddha:    { img:null,                    name:"ブッダ型",          nameEn:"Buddha",    sub:"悟りと慈悲の覚者",            color:"#f57f17", light:"#fffde7", desc:"苦しみの根源を見つめ、執着を手放すことで真の平和を得ようとする。自分だけでなくすべての存在の幸福を願い、静かな強さで世界と向き合う。", keyword:["慈悲","悟り","無我","平和"], emoji:"🪷" },
};
...
{"type":"key","second":"key or null","reason":"2-3 sentences in Japanese","match":"keyword under 15 chars","score":"high or medium or low"}`;
}
...
export default function App() {
  ...
  const diagnose = async () => {
    if (validPosts.length === 0) { setError("ポストを1つ以上入力してください"); return; }
  ...
  const shareText = result && t ? `私の思想タイプは「${t.name}」でした🪄\n${t.sub}${tSecond ? `\n（次点：${tSecond.name}）` : ""}\n\n#現人神アラジン #思想タイプ診断\nhttps://note.com/genial_ixia5647` : "";
    ...
    <div style={{ fontFamily: "var(--font-sans)", padding: "0 0 2rem" }}>
      ...
      {spinning && pendingResult && (
        <div style={{ paddingTop: "1rem" }}>
          <p style={{ color: "var(--color-text-secondary)", fontSize: "13px", textAlign: "center", margin: "0 0 8px" }}>🎡 タイプを判定中...</p>
      ...
      {!result && !spinning && (
        <>
          <div style={{ background: "#06061a", padding: "1.5rem", borderRadius: "0 0 24px 24px", marginBottom: "1.5rem" }}>
            ...
            <h1 style={{ color: "#FFD700", textAlign: "center", fontSize: "17px", fontWeight: "500", margin: "0 0 4px" }}>ポストで思想タイプ診断</h1>
            <p style={{ color: "#aaa", textAlign: "center", fontSize: "13px", margin: 0 }}>あなたはどんな偉人タイプ？</p>
          </div>
          <div style={{ padding: "0 1rem" }}>
            <p style={{ color: "var(--color-text-secondary)", fontSize: "13px", marginBottom: "12px" }}>
              Xのポストを貼り付けてください（{validPosts.length}/3）
            </p>
            {posts.map((p, i) => (
              <div key={i} style={{ marginBottom: "10px" }}>
                <textarea
                  ...
                  placeholder={["ポスト1を貼り付け", "ポスト2を貼り付け", "ポスト3を貼り付け（任意）"][i]}
            ...
            <button onClick={diagnose} disabled={loading || validPosts.length === 0}
              ...
              {loading ? "🪄 AIが分析中..." : "🪄 タイプを診断する"}
            </button>
            <p style={{ color: "var(--color-text-tertiary)", fontSize: "11px", textAlign: "center", marginTop: "12px" }}>12タイプの偉人から判定します</p>
          </div>
        </>
      )}
        ...
        <div style={{ padding: "0 1rem", paddingTop: "1rem" }}>
          <div style={{ background: "#06061a", borderRadius: "16px", marginBottom: "16px", textAlign: "center", border: `2px solid ${t.color}`, overflow: "hidden" }}>
            ...
            <div style={{ padding: "1.5rem" }}>
              ...
              {!t.img && <div style={{ fontSize: "3.5rem", marginBottom: "8px" }}>{t.emoji}</div>}
          ...
          {tSecond && (
            <div style={{ background: "var(--color-background-secondary)", borderRadius: "12px", padding: "1rem 1.25rem", marginBottom: "12px", border: `0.5px solid ${tSecond.color}55`, display: "flex", alignItems: "center", gap: "14px" }}>
              ...
              <div>
                <p style={{ color: "var(--color-text-tertiary)", fontSize: "11px", margin: "0 0 2px" }}>次点タイプ</p>
          ...
          <div style={{ background: "var(--color-background-secondary)", borderRadius: "12px", padding: "1.25rem", marginBottom: "12px", border: "0.5px solid var(--color-border-tertiary)" }}>
            <p style={{ color: "var(--color-text-secondary)", fontSize: "12px", margin: "0 0 8px", fontWeight: "500" }}>同じタイプの人と議論したい？</p>
            <p style={{ color: "var(--color-text-primary)", fontSize: "14px", margin: "0 0 12px", lineHeight: "1.6" }}>現人神アラジンのサロンでは、あなたの思想タイプが表示され、同じ・違うタイプの人と本気のアイデアをぶつけ合えます。🪄</p>
          ...
          <div style={{ background: "var(--color-background-secondary)", borderRadius: "12px", padding: "1.25rem", marginBottom: "12px", border: "0.5px solid var(--color-border-tertiary)" }}>
            <p style={{ color: "var(--color-text-secondary)", fontSize: "12px", margin: "0 0 8px", fontWeight: "500" }}>Xでシェアする</p>
          ...
          <button onClick={() => generateShareImage(t)}
          ...
          <button onClick={reset}
            style={{ width: "100%", padding: "12px", background: "none", border: "0.5px solid var(--color-border-secondary)", borderRadius: "10px", color: "var(--color-text-secondary)", fontSize: "14px", cursor: "pointer" }}>
            🪄 もう一度診断する
          </button>
        </div>
      )}
